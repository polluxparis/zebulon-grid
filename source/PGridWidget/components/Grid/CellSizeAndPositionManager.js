...
export default class CellSizeAndPositionManager {
  ...
  constructor ({cellCount, cellSizeGetter, estimatedCellSize}) {
    ...
    // Measuremenjs for cells up to this index can be trusted; cells afterward should be estimated.
   ...
   * This allows partially visible cells (with offsejs just before/above the fold) to be visible.
   */
  findNearestCell (offset) {
    if (isNaN(offset)) {
  ...
  getSizeAndPositionOfCell (index) {
    if (index < 0 || index >= this._cellCount) {
      throw Error(`Requested index ${index} is oujside of range 0..${this._cellCount}`)
    }
   ...
   * This method should be called for any cell that has changed ijs size.
