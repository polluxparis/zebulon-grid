...
// export function buildAxisLeaves(node, dimensions, measures) {
//   // console.log("buildLeaves0", Date.now());
//   const x = Date.now();
//   const leaves = getLeaves(node, dimensions, measures, 0, []);
//   console.log("buildAxisLeaves", leaves.length, Date.now() - x, node, leaves);
//   return leaves; // { nVisibles: headers.nVisibles, headers, leaves };
// }
// export const filterLeavesSelector = createSelector(
//   [
...
//     state.data.data
//   ],
//   (dimensions, rowAxis, columnAxis, rowLeaves, columnLeaves, data) => (dimensionId,filter){
//     if axisRows.include(dimensionId){rowLeaves}
//   }
// );
...
const expandCollapseAll = (node, depth, isCollapsed, measureCount) => {
  // console.log(header, depth);
  const leaves = getLeaves(node, [], depth);
  const keys = leaves.reduce(
    (acc, leaf) => {
      acc.n += expandCollapseNode(leaf, isCollapsed);
...
export const toggleSortOrderSelector = createSelector(
  ...
  (rowLeaves, columnLeaves, rowDimensions, columnDimensions) => (
    ...
    if (dimension.id !== TOTAL_ID && dimension.id !== MEASURE_ID) {
      ...
      leaves.leaves = getLeaves(leaves.node, [], false);
    }
  }
);
